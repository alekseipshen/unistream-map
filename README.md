...
```bash
python3 scripts/build_report.py            # report/Конкуренты_ЮНИСТРИМ_ГГГГ-ММ-ДД.xlsx
```

Лист «Сводка» — строка на отделение (сколько конкурентов, сколько выгоднее нас, лучшие курсы
рядом, окно перепродажи), дальше по листу на каждое отделение с полными курсами соседей.
Наши курсы тянутся живыми из API ЮНИСТРИМ на момент сборки.
